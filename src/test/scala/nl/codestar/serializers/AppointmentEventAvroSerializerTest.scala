...
class AppointmentEventAvroSerializerTest
    ...
    with FunSpecLike
    with MustMatchers {
  val serializer = new AppointmentEventAvroSerializer(
    this.system.asInstanceOf[ExtendedActorSystem])
  ...
  val validEvents = Table(
    ...
    ("appointment-2", AppointmentCreatedV2(randomUUID(), "subject", ZonedDateTime.now(), Some(BranchOfficeV1("12345")))),
    ("appointment-3", AppointmentCreatedV3(randomUUID(), "subject", ZonedDateTime.now(), Some(BranchOfficeV2("12345", 
      Some(randomUUID()))), Set("A", "B")))
  )
